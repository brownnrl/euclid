...
| `A.Point.followPath` | **IMPL** | `PointElement` | `{ via: [...] }`, `{ along: "DC" }`, `{ settle: bool }` | `1200 ms` | Move a point **along a path** for illustration (#137). Where `Point.slide` answers *"put this slider at that value"*, this answers *"watch this point travel"*. The path always begins at the point's **current** position, so it sets off from where it stands rather than teleporting to a start. Three path kinds, all optional and independent. **`via`** is a waypoint list of straight legs — each entry an element name (position read at setup) or an `[x, y]` / `[x, y, z]` **static coordinate**, freely mixed. **`along`** is sugar for a line or segment: appends its two endpoints. **`arc`** swings the point about a centre instead of walking legs, which reads far better for "carry this point to the other side" — a sweep rather than a V: `arc.center` is an element name or `[x, y]`, where a **point** is used as-is and a **line uses its MIDPOINT** (so `center: "DC"` means "swing about the middle of DC" with no midpoint element declared); `arc.radius` defaults to the point's **current** distance from the centre, so `t=0` leaves the figure exactly as authored; `arc.sweep` is **degrees to travel, default 180**, and its **sign picks the direction** (`-90` is the mirror of `90`). Legs are **arc-length parameterised**, so a long leg takes proportionally longer. `slate.update()` runs every frame, so dependents visibly react as the point moves — that reaction is the point of the animation. **`settle` (default `false`)** makes it an **out-and-back**: the point traverses the path over the first half and retraces it over the second, finishing *exactly* where it began, so a deck can illustrate a failure mode without disturbing the figure. `settle: true` ends at the far end. Motivating case is **I.31** — walk the apex across `DC` so the "parallel" visibly breaks, then restore. Aimed at **free points**: a constrained point re-projects onto its own locus in `update()`, so driving one off its line does nothing useful. Bad waypoints / a non-line `along` / no usable path each report a diagnostic (#154) and leave the point put. |
